...
void ObstacleWorld::dispose() {
    clear();
    if (_real_world != nullptr) {
        delete _real_world;
        _real_world  = nullptr;
    }
    if (_draw_world != nullptr) {
        delete _draw_world;
        _draw_world = nullptr;
    }
    onBeginContact = nullptr;
...
bool ObstacleWorld::init(const Rect bounds, const Vec2 gravity) {
    ...
    _bounds = bounds;
    _real_world = new b2World(b2Vec2(gravity.x,gravity.y));
    _draw_world = new b2World(b2Vec2(gravity.x, gravity.y));
    if (_real_world && _draw_world) {
        return true;
    }
    return false;
}
...
void ObstacleWorld::addObstacle(const std::shared_ptr<Obstacle>& obj) {
    ...
    obj->activatePhysics(*_real_world, *_draw_world);
}
...
void ObstacleWorld::removeObstacle(Obstacle* obj) {
    for(auto it = _objects.begin(); it != _objects.end(); ++it) {
        if (it->get() == obj) {
            obj->deactivatePhysics(*_real_world, *_draw_world);
            _objects.erase(it);
            return;
        }
    }
...
void ObstacleWorld::garbageCollect() {
    ...
    for(size_t ii = 0; ii < _objects.size(); ii++) {
        if (_objects[ii]->isRemoved()) {
            _objects[ii]->deactivatePhysics(*_real_world, *_draw_world);
            _objects[ii] = nullptr;
        } else {
            if (pos != ii) {
...
void ObstacleWorld::setGravity(const Vec2 gravity) {
    _gravity = gravity;
    if (_real_world != nullptr) {
        _real_world->SetGravity(b2Vec2(gravity.x,gravity.y));
    }
    if (_draw_world != nullptr) {
        _draw_world->SetGravity(b2Vec2(gravity.x, gravity.y));
    }
}
...
void ObstacleWorld::activateCollisionCallbacks(bool flag) {
    ...
    _real_world->SetContactListener(flag ? this : nullptr);
    _collide = flag;
}
...
void ObstacleWorld::activateFilterCallbacks(bool flag) {
    ...
    _real_world->SetContactFilter(flag ? this : nullptr);
    _filters = flag;
}
...
void ObstacleWorld::activateDestructionCallbacks(bool flag) {
    ...
    _real_world->SetDestructionListener(flag ? this : nullptr);
    _destroy = flag;
}
...
void ObstacleWorld::queryAABB(std::function<bool(b2Fixture* fixture)> callback, const Rect aabb) const {
    ...
    _real_world->QueryAABB(&proxy, b2box);
}
